...
iou_thresholds = [x / 100 for x in range(50, 100, 5)]    # [0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95]
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45):
    ...
    backcolor = {'white': 1, 'red': torch.tensor([1, 0, 0], dtype=torch.float),
             'green': torch.tensor([0, 1, 0], dtype=torch.float), 'blue': torch.tensor([0, 0, 1], dtype=torch.float),
             'black': 0}
    if args.display_masks and cfg.eval_mask_branch:
        # After this, mask is of size [num_dets, h, w, 1]
        masks = masks[:num_dets_to_consider, :, :, None]
        mask_n = torch.zeros(masks[0].shape)
        for num in range(num_dets_to_consider):
            mask_n = mask_n + masks[num]
        mask_n = torch.clamp(mask_n, 0, 1)
        #mask = 1 - mask_n.repeat(1, 1, 3).type(torch.uint8)
        mask = mask_n.repeat(1, 1, 3).type(torch.uint8)
        temp_img = img_gpu.clone()
        temp_img[mask[:, :, 0]] = backcolor[args.backcolor]
        img_numpy = (temp_img * 255).byte().cpu().numpy()
        mask0 = mask[:, :, 0].byte().cpu().numpy()
        image_mask = Image.fromarray(mask0*255)
        image_mask.save("./result/back_mutation/000000118561/000000118561_mask.png")
    return img_numpy
...
def evalimage(net:Yolact, path:str, save_path:str=None):
    ...
    frame = torch.from_numpy(image2).cuda().float()
    ...
    if not os.path.exists('result/back_mutation'):
        os.makedirs('result/back_mutation')
    if not args.images:
        n = args.image.split("/")[-1].split(".")[0]
        save_path = "./result/back_mutation/" + n + "/"
        os.system("mkdir " + save_path)
        image = Image.fromarray(img_numpy)
        save_path = "./result/back_mutation/" + n + "/" + n + '_' + args.backcolor +'.png'
...
def evalimages(net:Yolact, input_folder:str, output_folder:str):
    ...
    for p in Path(input_folder).glob('*'):
        ...
        name = '.'.join(name.split('.')[:-1]) + '_' + args.backcolor + '.png'
