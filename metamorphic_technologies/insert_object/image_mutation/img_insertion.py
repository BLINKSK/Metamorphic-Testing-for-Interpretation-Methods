...
def delta_insertion(win, cl, c):
    ...
    os.system("mv new_" + str(idx) + ".png " + "../image_insertion/new_final_" + str(c) + ".png")
...
def process():
    ...
    while True:
        ...
        insert_object_images(x, y, 0)
            # find one
        os.system("mv new_0.png ../image_insertion/new_start_" + str(c) + ".png")
        c += 1
        win_list.append((x, y))
        # break
...
def main(n, obj, width, length, c):
    ...
    if (check_label_consistency() == False):
        ...
        return
    process()
    with cd("../image_insertion/"):
        if not os.path.exists(num):
            os.system("mkdir " + num)
        os.system("mkdir " + num + '/' + n + "-" + obj)
        os.system("cp ../image_pool/" + n.split('.')[0] + "/object.log " + num + '/' + n + "-" + obj + "/")
