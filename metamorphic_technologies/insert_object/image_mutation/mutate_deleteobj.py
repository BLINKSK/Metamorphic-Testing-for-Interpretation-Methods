...
def smoothing(imgpath, name, boxs):
    ...
    for key in list(boxs.keys()):
        ...
        if len(arry_image.shape) == 3:
            ...
            image1 = image * (1 - mask)
        image_zsm = image_sm + image1
        # print(image_zsm)
        image_zsm = np.uint8(image_zsm)
        image_zsm = Image.fromarray(image_zsm)
        image_sp = image_zsm.filter(ImageFilter.SHARPEN)
        image_sp = image_sp.filter(ImageFilter.SHARPEN)
        image_sp = image_sp.filter(ImageFilter.SHARPEN)
        image_sp = image_sp.filter(ImageFilter.SHARPEN)
        image_sp = image_sp.filter(ImageFilter.SHARPEN)
        if len(arry_image.shape) == 3:
            image_sp = image_sp * mask.repeat(3, 1).reshape(image.size[1], image.size[0], arry_image.shape[2])
        else:
            image_sp = image_sp * mask
        image_zz = image_sp + image1
