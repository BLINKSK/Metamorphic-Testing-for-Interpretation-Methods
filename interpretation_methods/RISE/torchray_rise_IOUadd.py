...
def gradcam(image_path, net, indexnames, names, boxs, nums, name_num, oricam, oribox, oriboxkey, insertion):
    ...
    for indexname in indexnames:
        ...
        if conf > 0.6 or oribox == None:
            saliency = rise(net, img_input, num_masks=10000, batch_size=128)
            saliency = saliency[:, int(indexnum)].unsqueeze(0)
            ...
            if not np.isnan(camv):
                ...
                ret, thresh1 = cv2.threshold(cam, 0.55, 1, 0)
                ...
                if insertion:
                    ...
                    if (thre < 0.5) and (threc < 0.5) and (max_thre_key[0] < 0.5):
                        if not os.path.exists("IOUin" + args.input_number + "_error/" + name0):
                            os.system("mkdir IOUin" + args.input_number + "_error/" + name0)
                        os.system(
                    ...
                    if (thre < 0.5) and (threc >= 0.5):
                        if not os.path.exists("IOUin" + args.input_number + "_error_camsmall/" + name0):
                            os.system("mkdir IOUin" + args.input_number + "_error_camsmall/" + name0)
                        os.system(
                    ...
                    if (thre < 0.5) and (max_thre_key[0] >= 0.5):
                        if not os.path.exists("IOUin" + args.input_number + "_single_object/" + name0):
                            os.system("mkdir IOUin" + args.input_number + "_single_object/" + name0)
                        os.system(
                        ...
                        if object_add != indexname:
                            if (camo_value / cam_value < 0.5 and camo_value / oricam_value0 < 0.5):
                                if not os.path.exists("IOU" + args.input_number + "_change/" + name0):
                                    os.system("mkdir IOU" + args.input_number + "_change/" + name0)
                    ...
                    if conf > 0.6:
                        if ((thre < 0.5) and (max_thre_key[0] >= 0.5)):
                            if not os.path.exists("IOU" + args.input_number + "_single_object/" + names):
                                os.system("mkdir IOU" + args.input_number + "_single_object/" + names)
                                os.system(
                        ...
                        if ((thre < 0.5) and (threc < 0.5) and (max_thre_key[0] < 0.5)):
                            if not os.path.exists("IOU" + args.input_number + "_error/" + names):
                                os.system("mkdir IOU" + args.input_number + "_error/" + names)
                            os.system(
                        ...
                        if (thre < 0.5) and (threc >= 0.5):
                            if not os.path.exists("IOU" + args.input_number + "_error_camsmall/" + names):
                                os.system("mkdir IOU" + args.input_number + "_error_camsmall/" + names)
                            os.system(
...
if __name__ == '__main__':
    ...
    for p in images_remain:
        ...
        if os.path.exists(objectlog):
            ...
            c = c + 1
    print('error:', c)
    # for ic in list(camori[namesori].keys()):
    #     camori[namesori][ic] = camori[namesori][ic].tolist()
    # json_str = json.dumps(camori)
    # with open('gradcamori.json', 'a') as json_file:
    #     json_file.write(json_str + '\n')
    # for ib in list(boxori[namesori].keys()):
    #     boxori[namesori][ib] = boxori[namesori][ib].tolist()
    # json_str0 = json.dumps(boxori)
    # with open('boxori_val.json', 'a') as json_file:
    #     json_file.write(json_str0 + '\n')
    # for ibb in list(boxkey[namesori].keys()):
    #     boxkey[namesori][ibb] = boxkey[namesori][ibb].tolist()
    # json_str1 = json.dumps(boxkey)
    # with open('boxkey_val.json', 'a') as json_file:
