...
def gradcam(image_path, net, indexnames, names, boxs, oricam, oribox, oriboxkey, insertion):
    img_path = image_path
            ...
            if not np.isnan(camv):
                ...
                if insertion:
                    ...
                    name = name1 + '_' + indexname + '_' + str(conf) + '_' + str(
                        np.round(thre, 3)) + '_' + str(np.round(threc, 3)) + '_' + str(np.round(max_thre_key[0], 3)) + '.png'
            ...
            for p0 in Path(path).glob('*'):
                ...
                if not boxs:
                    with open(objectlog) as f:
                        ...
                        camori, boxori, boxkey = gradcam(imagepathori, model, indexnames, namesori, boxs, None, None, None, insertion=False)
                # print(boxkey)
                # squ_name = (names.split('_')[1]).split('.')[0]
                # print('class_num', class_num)
                gradcam(path0, model, indexnames, names, boxs, camori, boxori, boxkey, insertion=True)
        else:
            c = c + 1
    print('error:', c)